...
.Passos
. Repita os passos 1 a 3 na link:veritas-configure-storagegrid-s3-object-lock.html#configure-storagegrid-s3-object-lock-default-bucket-retention["Configuração básica"] seção, mas desta vez selecione a opção Armazenar dados no modo WORM usando S3 Object Lock. Clique em Avançar.
+
