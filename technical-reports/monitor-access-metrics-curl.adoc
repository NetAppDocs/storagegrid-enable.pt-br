...
----
+
Agora você pode usar a string do token de autorização para acessar métricas por meio do cURL. O processo para acessar métricas é semelhante às etapas da seção link:monitoradvanced-monitor-storagegrid.html#export-metrics-through-the-api["Monitoramento avançado em StorageGRID"]. No entanto, para fins de demonstração, mostramos um exemplo com GET /grid/metric-labels/{label}/values selecionado na categoria Métricas.
