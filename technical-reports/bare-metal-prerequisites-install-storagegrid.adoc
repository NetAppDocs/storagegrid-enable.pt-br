...
NOTE: Os números na tabela referem-se a cada tipo de serviço StorageGRID, não à grade inteira ou a cada host físico. Com base nas opções de implantação, calcularemos os números para cada host físico em link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["Layout e requisitos físicos do host"], posteriormente neste documento. ++ ++ Os caminhos ou sistemas de arquivos marcados com um asterisco serão criados no próprio contêiner StorageGRID pelo instalador. Nenhuma configuração manual ou criação de sistema de arquivos é necessária por parte do administrador, mas os hosts precisam de dispositivos de bloco para atender a esses requisitos. Em outras palavras, o dispositivo de bloco deve aparecer usando o comando  `lsblk`, mas não deve estar formatado ou montado no sistema operacional do host. ++ ++
...
|===
Neste exemplo, os tamanhos de disco mostrados na tabela a seguir são necessários por tipo de contêiner. Os requisitos por host físico são descritos em link:bare-metal-prerequisites-install-storagegrid.html#physical-host-layout-and-requirements["Layout e requisitos físicos do host"], mais adiante neste documento.
...
Se você não estiver usando storage iSCSI, é necessário garantir que host1, host2 e host3 contenham dispositivos de bloco com tamanho suficiente para atender aos seus requisitos. Consulte link:bare-metal-prerequisites-install-storagegrid.html#disk-sizes-per-container-type["Tamanhos de disco por tipo de contentor"] para os requisitos de storage de host1, host2 e host3.
...
----
. Usando o nome do iniciador da etapa 2, mapeie os LUNs em seu dispositivo de storage (no número e tamanho mostrados na link:bare-metal-prerequisites-install-storagegrid.html#storage-requirements["Requisitos de storage"] tabela) para cada nó de storage.
